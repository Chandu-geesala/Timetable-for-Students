...
class _TimetablePageViewState extends State<TimetablePageView> {
  ...
  bool _autoNavigate = true; // Flag to control automatic navigation
  ...
  void initState() {
    ...
    WidgetsBinding.instance!.addPostFrameCallback((_) {
      if (_autoNavigate) {
        _autoNavigateToPresentDaySlide();
      }
    });
  }

  void _autoNavigateToPresentDaySlide() {
    // Get current day of the week (Monday is 1, Tuesday is 2, ..., Saturday is 6)
    final currentDayOfWeek = DateTime.now().weekday;

    // Map current day to slide index (Monday is 0, Tuesday is 1, ..., Saturday is 5)
    final slideIndex = currentDayOfWeek - 1;

    if (slideIndex >= 0 && slideIndex < 6) {
      _pageController.jumpToPage(slideIndex);
      setState(() {
        _currentPageIndex = slideIndex;
        _autoNavigate = false; // Reset auto-navigation flag
      });
      _preloadData(slideIndex - 1); // Preload data for previous day
      _preloadData(slideIndex);     // Preload data for current day
      if (slideIndex < 5) {
        _preloadData(slideIndex + 1); // Preload data for next day
      }
    } else {
      // Handle error: Slide index out of range
                ...
                children: [
                  ...
                  Positioned(
                    ...
                    child: FloatingActionButton(
                      ...
                      onPressed: () async {
                        final List<ClassEvent> existingEvents = _classEventsCache[_currentPageIndex] ?? [];

                        final int newClassNumber = existingEvents.length + 1;
                        final ClassEvent newClassEvent = ClassEvent(
                          day: _getDayName(_currentPageIndex),
                          className: 'Class $newClassNumber',
                          startTime: '00:00',
                          endTime: '00:00',
                        );
                        ...
                        await ClassEventDatabaseHelper().insertClassEvent(newClassEvent);

                        // Update the class events cache for the current day slide
                        _classEventsCache[_currentPageIndex] = [...existingEvents, newClassEvent];
                      ...
                      child: Container(
                        decoration: BoxDecoration(
                          ...
                          image: DecorationImage(
                            image: AssetImage('assets/gemini.webp'), // Replace 'your_image.png' with your image asset path
                            fit: BoxFit.cover,
                          ),
                        ),
                      ),
  ...
  Future<void> _deleteClassEvent(ClassEvent classEvent) async {
    ...
    if (confirmDelete != null && confirmDelete) {
      try {
        ...
        // Update the class events cache for the current day slide
        if (_classEventsCache.containsKey(_currentPageIndex)) {
          _classEventsCache[_currentPageIndex]!.removeWhere((event) => event.id == classEvent.id);
        }

        // Forcefully refresh the UI
        setState(() {});
      } catch (e) {
        print("Error deleting class event: $e");
